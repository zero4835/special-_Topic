...
def writeExcel(text, user, location):
    ...
    try:
        old_data = pd.read_excel('test.xlsx')
    except FileNotFoundError:
        # 若找不到原有檔案，則新建一個 DataFrame
    ...
    #write back excel
    merged_data.to_excel('./test.xlsx', index=False)
...
text6 = '日日旅海外面還有個大草坪上面有一台可愛的小餐車住客可以免費兌換任何品項有冰淇淋、氣泡飲、咖啡???如果只是一般遊客在營業時間入場費90 可以抵消費有去墾丁走走很推這個位在滿州鄉的民宿遠離塵囂，非常靜謐又放鬆～??' 
text7 = '今年大棠紅葉?特別靚特別多由9:30am開始上山到3:30pm才下到山???♀?全個元朗大擠塞人山人海好似行年宵咁誇張??♀????♂???♀????♂???♀????♂???我?去景點?「楓香林?」遊覽打卡波波車一身牛仔打扮成為流動景點被打卡?基本上六個鐘頭有四個鐘頭都係背住波波今日負重7kg行山運動6小時?絕對是體力耐力?堅持同配合???♀?玩得開心?影到靚相都係值得??'
